...
public class RecetaResource {
    ...
    public RecetaResource(RecetaService recetaService, RecetaRepository recetaRepository, RecetaQueryService recetaQueryService) {
    ...
    public ResponseEntity<RecetaDTO> createReceta(@Valid @RequestBody RecetaDTO recetaDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/recetas/" + recetaDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, recetaDTO.getId().toString()))
            .body(recetaDTO);
    }
    ...
    public ResponseEntity<RecetaDTO> partialUpdateReceta(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody RecetaDTO recetaDTO
    ) throws URISyntaxException {
