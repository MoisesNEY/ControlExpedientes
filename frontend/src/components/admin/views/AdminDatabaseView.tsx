...
const AdminDatabaseView = () => {
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            setMessage('Respaldo generado y almacenado correctamente.');
            await loadSummary();
        } catch (error) {
    ...
    const handleSaveSettings = async () => {
        ...
        try {
            const updated = await DatabaseAdminService.saveSettings({
                ...settings,
                time: normalizeTimeValue(settings.time),
            });
            setSettings({
                ...updated,
                time: normalizeTimeValue(updated.time),
            });
            setSummary((current) => current ? { ...current, settings: updated } : { settings: updated, backups: [] });
    ...
    const handleRestoreUpload = async () => {
        ...
        if (!window.confirm('Esta acción reemplazará la base de datos actual. ¿Desea continuar?')) {
            return;
        }
    ...
    const handleRestoreStoredBackup = async (backup: DatabaseBackupHistoryItem) => {
        if (!window.confirm(`Se restaurará la base de datos usando ${backup.filename}. ¿Desea continuar?`)) {
            return;
        }
        ...
        try {
            await DatabaseAdminService.restoreStoredBackup(backup.filename);
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto w-full flex flex-col gap-6 md:gap-8">
            <div className="flex flex-col gap-1">
                ...
                <p className="text-slate-500 text-base font-medium">
                    Exporte, restaure y programe respaldos automáticos desde este módulo administrativo.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6">
                ...
                <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm">
                    ...
                    <p className="mt-2 text-sm text-slate-500">Respaldos disponibles en el servidor para descarga o restauración.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-4">
                    ...
                    <AppButton variant="outline" icon="upload" disabled={!backupFile} isLoading={restoringUpload} onClick={handleRestoreUpload}>
            ...
            <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-5">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-slate-500">Defina el día, la hora y la frecuencia con la que el sistema debe exportar la base de datos automáticamente.</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/40 p-4">
                        ...
                        <p className="mt-2 text-xs text-slate-500">
                            {FREQUENCY_OPTIONS.find((option) => option.value === settings.frequency)?.description}
                        </p>
                    </div>
            ...
            <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm space-y-4">
                ...
                {loading ? (
                                ...
                                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <AppButton
                                            ...
                                            onClick={() => void handleDownloadStoredBackup(backup)}
                                        >
                                            Descargar
                                        </AppButton>
                                        <AppButton
                                            ...
                                            onClick={() => void handleRestoreStoredBackup(backup)}
                                        >
                                            Restaurar
                                        </AppButton>
                                    </div>
                    ...
                    <p className="text-sm text-slate-500">Todavía no hay respaldos almacenados en el servidor.</p>
                )}
            </div>
